...
class PuzzleState:
    ...
    def calculate_cost(self, goal_state):
        ...
        for i in range(len(self.board)):
            for j in range(len(self.board[i])):
                if self.board[i][j] != goal_state.board[i][j]:
                    cost += 1
        self.cost = cost


def solve_puzzle_branch_and_bound(initial_state, goal_state):
    queue = PriorityQueue()
    ...
    while not queue.empty():
        ...
        for move in possible_moves:
            ...
            if new_state not in visited:
                new_state.calculate_cost(goal_state)
                queue.put(new_state)
...
solution = solve_puzzle_branch_and_bound(initial_state, goal_state)
